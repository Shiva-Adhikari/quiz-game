...
def random_quiz(request: StartQuizRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    # ✅ FIXED: Filter questions by difficulty if specified
    filters = [Question.is_active]
    
    # Add difficulty filter if not mixed (null means mixed)
    if request.difficulty_level and request.difficulty_level.upper() != 'MIXED':
        from src.utils.enums import DifficultyLevel
        try:
            difficulty_enum = DifficultyLevel[request.difficulty_level.upper()]
    ...
    query = db.query(Question).filter(*filters)
    ...
    if available_count < request.total_questions:
        ...
        random_questions = query.all()
    else:
        # Get random questions
        random_questions = query.order_by(func.random()).limit(request.total_questions).all()

    if not random_questions:
        raise HTTPException(status_code=404, detail='No questions available with these filters')
    ...
    # ✅ FIXED: Use the actual difficulty from request
    ...
    quiz_session = QuizSession(
        ...
        category_id=None,
        session_type='random',
        difficulty_level=difficulty_to_save,  # ✅ Dynamic difficulty
        total_questions=len(random_questions),  # ✅ Use actual count
        current_question_index=0,
        questions_answered=0,
        correct_answers=0,
        status=SessionStatus.STARTED,
    ...
    return {
        ...
        "questions": questions_response,
        "timer_expires_at": timer_expires_at,
        "message": f"Quiz started successfully! You have {len(random_questions)} questions to answer."
    }
